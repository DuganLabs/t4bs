import { json, error, readJson, requireAdmin, visitorZone } from "../../_shared/util.js";
...
/* GET /api/admin/schedule?past=14&ahead=30
   ...
   "Today" is the ADMIN's own day, from `request.cf.timezone` via
   visitorZone() — never from a param, for the reason in
   shared/daily.js's header. `past`/`ahead` are window sizes and are
   safe to take from the query string; the day key itself is not. The
   schedule is one row per calendar date for everybody, so an admin in
   Chicago filling ahead and an admin in Auckland filling ahead write
   the same rows — at most they start from adjacent dates. */
export const onRequestGet = async ({ request, env }) => {
  ...
  const today = zonedDayKey(new Date(), visitorZone(request));
...
export const onRequestPut = async ({ request, env }) => {
  ...
  /* Past relative to the admin's own date — a pin for "tomorrow" must
     not be refused because the edge is already past midnight. */
  if (day < zonedDayKey(new Date(), visitorZone(request))) return error("day-in-past", 400);
